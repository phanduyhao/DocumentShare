                                        ...
                                        @foreach($searchResults as $searchResult)
                                            <div class="col-lg-4 col-md-3 col-sm-6 col-12 mb-3">
                                                <div class="content-product-item position-relative">
                                                    <a href="" data-id="{{$searchResult->id}}" class="position-absolute end-0 z-3 me-2 mt-2 btn-favourite">
                                                        <i class="fa-solid fa-heart icon-favourite {{ $favourites->contains('document_id', $searchResult->id) ? 'text-danger' : '' }}"></i>
                                                    </a>
                                                    <div class="content-product-item-titel">
                                                        <h4 class="me-3">{{$searchResult->title}}</h4>
                                                        <div class="subject d-flex align-items-center">
                                                            <img
                                                                ...
                                                                class="me-2"
                                                            />
                                                            <span>{{$searchResult->Category->title}}</span>
                                                        </div>
                                                    </div>
                                                    <div class="position-relative">
                                                        <div
                                                            ...
                                                            <div class="view">
                                                                <i class="ti-eye"></i>
                                                                <span>{{ $searchResult->views_count }}</span>
                                                            </div>
                                                            <div class="download d-flex align-items-center">
                                                                <i class="fa-solid fa-download pe-1"></i>
                                                                <span>{{ $searchResult->downloads_count }}</span>
                                                            </div>
                                                        </div>
                                                        <div class="author">
                                                            <i class="fa-solid fa-user"></i>
                                                            <span>Tác giả: {{$searchResult->User->name}}</span>
                                                        </div>
                                                        <div class="action-click">
                                                            <div class="action-read-download">
                                                                <div class="action-read-download-btn me-2">
                                                                    <a data-id="{{$searchResult->id}}" class="btn-show__details-file" target="_blank"
                                                                       href="{{ route('documentMain.details', ['slug' => $searchResult->slug]) }}">Đọc online</a>
                                                                </div>
                                                                <div class="action-read-download-btn">
                                                                    <a data-id="{{$searchResult->id}}" data-score-doc="{{$searchResult->score}}" @auth data-score-user="{{ Auth::user()->score }}" data-user-id="{{ Auth::id() }}" @endauth href="/temp/filesOrigin/{{$searchResult->file}}.{{$searchResult->type}}" download class="download-file">Tải xuống</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
